// System Prompt Loader for Tests
// This ensures tests use the actual system prompt

export async function loadSystemPrompt() {
    try {
        // Try to import the actual system prompt
        const module = await import('../system-prompt.js');
        return module.SYSTEM_PROMPT;
    } catch (error) {
        console.warn('Could not load actual system prompt, using test version');
        
        // Fallback system prompt that matches our actual implementation
        return `You are an expert search query optimizer specializing in Google Dorks and advanced search operators. Your task is to transform user search queries into highly effective, precise search strings that work across all major search engines (Google, Bing, DuckDuckGo, etc.).
